...
function loginError(message: string, step?: "otp"): never {
  const suffix = step === "otp" ? "&step=otp" : "";

  redirect(`/admin/login?error=${encodeURIComponent(message)}${suffix}`);
}
...
export async function login(formData: FormData) {
  ...
  if (!userId) {
    loginError("The administrator session could not be created.");
  }
  ...
  const { code, token } = createAdminOtpChallenge(userId, email);

  const delivery = await sendAdminLoginOtp(email, code);
  ...
  cookieStore.set(ADMIN_OTP_CHALLENGE_COOKIE, token, {
    httpOnly: true,
    secure: secureCookie(),
    sameSite: "lax",
    path: "/",
...
export async function verifyAdminOtp(formData: FormData) {
  ...
  if (code.length !== 6) {
    loginError("Enter the complete six-digit verification code.", "otp");
  }

  const cookieStore = await cookies();

  const result = verifyAdminOtpChallenge(
    cookieStore.get(ADMIN_OTP_CHALLENGE_COOKIE)?.value,
    code,
  );

  if (!result.ok) {
    if (result.reason === "incorrect" && result.token) {
      cookieStore.set(ADMIN_OTP_CHALLENGE_COOKIE, result.token, {
        httpOnly: true,
        secure: secureCookie(),
        sameSite: "lax",
        path: "/",
    ...
    cookieStore.delete(ADMIN_OTP_CHALLENGE_COOKIE);
  ...
  if (!userResult.user || userResult.user.id !== result.challenge.userId) {
    cookieStore.delete(ADMIN_OTP_CHALLENGE_COOKIE);

    loginError("Your administrator session expired. Sign in again.");
  }

  const { data: administrator } = await supabase
    .from("admin_users")
    .select("is_active")
    .eq("user_id", result.challenge.userId)
    .maybeSingle();

  if (!administrator || !administrator.is_active) {
    await supabase.auth.signOut();

    loginError("This administrator account is no longer active.");
  }
  ...
  cookieStore.set(
    ADMIN_MFA_COOKIE,
    createAdminVerification(result.challenge.userId),
    {
      httpOnly: true,
      secure: secureCookie(),
      sameSite: "strict",
