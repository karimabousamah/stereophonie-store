...
function getMemoryType(
  ...
  if (
    direct.includes("DDR4") &&
    direct.includes("DDR5")
  ) {
    return "DDR4 / DDR5";
  }
  ...
  if (
    haystack.includes("DDR5")
  ) {
    return "DDR5";
  }

  if (
    haystack.includes("DDR4")
  ) {
    return "DDR4";
  }
...
function getMaxMemory(
  ...
  return numberValue(
    p.maxMemoryGb ??
    p.maxMemoryCapacityGb,
  );
}

function getPsuWattage(
  part:
    | PcPart
    | undefined,
) {
...
function getSupportedFormFactors(
  part:
    | PcPart
    | undefined,
) {
  ...
  return stringArray(
    p.supportedFormFactors,
  ).map(upper);
}

function getGpuLength(
  part:
    | PcPart
    | undefined,
) {
...
function getSupportedRadiators(
  part:
    | PcPart
    | undefined,
) {
