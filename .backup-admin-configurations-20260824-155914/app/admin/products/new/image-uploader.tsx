  ...
  const namedConfigurations =
    configurations
      .map((configuration) => ({
        ...configuration,
        variant_name:
          configuration.variant_name.trim(),
      }))
      .filter(
        (configuration) =>
          configuration.variant_name.length > 0,
      );
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-4">
                      ...
                      <select
                        ...
                        className="mt-2 min-h-12 w-full border border-white/10 bg-black/30 px-3 text-sm text-white outline-none transition focus:border-white/40"
                      >
                        <option value="">
                          Shared with all configurations
                        </option>

                        {namedConfigurations.map(
                          (configuration) => (
                            <option
                              key={configuration.clientId}
                              value={configuration.variant_name}
                            >
                              Only {configuration.variant_name}
                            </option>
                          ),
                        )}
                      </select>
                      ...
                      <p className="mt-2 text-[11px] leading-5 text-white/30">
                        Shared photographs appear for every option. Choose one
                        configuration when this image belongs only to that
                        exact version.
                      </p>
                    </div>
